...
def load_and_process_data(_file, file_identifier):
    try:
        ...
        # --- Ensure analyte column is always mapped correctly ---
        analyte_candidates = [col for col in df.columns if col.strip().upper() in ['ANALYT', 'ANALYTE', 'PARAMETER', 'STOFF']]
        if analyte_candidates:
            df.rename(columns={analyte_candidates[0]: 'analyte'}, inplace=True)
